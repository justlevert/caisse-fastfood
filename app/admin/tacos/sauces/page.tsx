...
export default function AdminTacoSaucesPage() {
  ...
  const [editingSauce, setEditingSauce] = useState<TacoSauce | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingSauce) {
      ...
      if (!error) {
        alert('Sauce modifiée avec succès');
        loadSauces();
        closeModal();
      } else {
      ...
      const { error } = await supabase.from('taco_sauces').insert(sauceData);

      if (!error) {
        alert('Sauce ajoutée avec succès');
        loadSauces();
        closeModal();
      } else {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Êtes-vous sûr de vouloir supprimer cette sauce ?')) return;

    const { error } = await supabase.from('taco_sauces').delete().eq('id', id);

    if (!error) {
      alert('Sauce supprimée');
      loadSauces();
    } else {
  ...
  const openModal = (sauce?: TacoSauce) => {
    if (sauce) {
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">Sauces</h1>
              ...
              <tbody className="divide-y divide-gray-200">
                {sauces.map((sauce) => (
                  <tr key={sauce.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 text-sm font-medium text-gray-900">{sauce.nom}</td>
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-semibold ${
                          sauce.actif ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                        }`}
                      >
                        {sauce.actif ? 'Actif' : 'Inactif'}
                      </span>
                    </td>
