...
import { TacoIngredient } from '@/types/database.types';

export default function AdminTacoIngredientsPage() {
  const [ingredients, setIngredients] = useState<TacoIngredient[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingIngredient, setEditingIngredient] = useState<TacoIngredient | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingIngredient) {
      ...
      const { error } = await supabase.from('taco_ingredients').insert(ingredientData);
  ...
  const handleDelete = async (id: string) => {
    ...
    const { error } = await supabase.from('taco_ingredients').delete().eq('id', id);
  ...
  const openModal = (ingredient?: TacoIngredient) => {
    if (ingredient) {
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">Ingrédients à retirer</h1>
              ...
              <tbody className="divide-y divide-gray-200">
                {ingredients.map((ingredient) => (
                  <tr key={ingredient.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 text-sm font-medium text-gray-900">{ingredient.nom}</td>
                    <td className="px-6 py-4">
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-semibold ${
                          ingredient.actif ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                        }`}
                      >
